...
const AdminNotification = () => {
  ...
  const handleSendNotification = async () => {
    ...
    const notificationData = {
      email: sendAsBulk ? 'bulk' : selectedUser,
      message: message,
      role: sendAsBulk ? 'bulk' : peers.some((peer) => peer.email === selectedUser) ? 'peer' : admins.some((admin) => admin.email === selectedUser) ? 'admin' : 'user',
      department: sendAsBulk
        ? null
        : peers.find((peer) => peer.email === selectedUser)?.dept || null,
    };
                ...
                >
                  <MenuItem value="" disabled>
                    {peers.length === 0 && users.length === 0 && admins.length === 0
                      ? 'No users, peers, or admins available'
                      : 'Select User'}
                  </MenuItem>
                  {peers.map((peer) => (
